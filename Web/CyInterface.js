...
var ctx = cyCanvas.getContext('2d');           //返回了一个在画布上绘图的环境
...
function modelChange(model) {
            ...
            if (newCmd[1] == "Info") {
                ...
                if (newCmd[2] == "KeyModel") {
                    ...
                    if (selected_model == 6 || selected_model == 5) {
                        //selectINSIGHT.style.visibility = 'hidden'; //default hidden
                        //selectEPOC.style.visibility = 'hidden';   //default visible
    ...
    client.onData  = function(text) {
        scroll_screen();
        var eeg_resolution = (document.getElementById("myRange").value * .01);    //default 50 * 0.01
        ...
        if (selected_model == 6 || selected_model == 5) {
            ...
            if (contact[1] == viewType) {
                    ...
                    //Plot Multiple Raw Data values.
                        ...
                        for (var i = 2; i < (contact.length -4); i+=2) {
                            ...
                            if (c.checked == true) {
                                ...
                                var value_data = convertEPOC_PLUS(parseInt(value_1), parseInt(value_2));  //parseInt将各种数据格式的string转化为整数
                ...
                if (contact[1] == "16") {              // contact = text.split(delimiter);
                    if (baseline[1] == null || reset_baseline == true) {
                        //console.log("Reset Baseline.");
                        if (manualControl) {           // default not checked!!!!!!!!!!    
                            if (formatType == 0) {
